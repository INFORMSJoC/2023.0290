...
protected:
	...
	//unsigned short K = NUM_K;
	unsigned short K;
	unsigned short Nx, Nw, Ny, Nxi, Nzeta; // Variables.
	unsigned short Lx, Ly, Lw, Lxi, R; // Number of cosntraints for X, Y, W and in the Uncertainty Set (Lxi). and deterministic constraints linking X, Y, and W.
	...
	bool decision_dependent_discovery; /*True if ddi*/
	char results_folder[1024]; /*Folder in which results will be collected*/
	FILE* out_folder;
...
*/
	...
	*/
	Matrix2D<double> C; // Uncertainty  vector cost for x.
	Matrix2D<double> D; // Uncertainty  vector cost for w.
	Matrix2D<double> Q; // Uncertainty vector cost for y.
...
Uncertainty set
	...
	*/
	...
	/*valid bounds*/
	virtual bool define_valid_bounds() = 0;
...
public:
	...
	ProblemDef() = default; // Default constructor.

	~ProblemDef() = default; // Default distructor.
	...
	*/
	...
	double getMaxNumDiscoveryFraction();
	void setMaxNumDiscoveryFraction(double mmdf);

	bool getDDIDversion();
	void setDDIversion(bool decision_dependent_discovery);
