...
struct SPP {
	...
	/*Discovery fraction Delta \in [0,1]*/
	double Delta;
 ...
 */
static inline void gen_SPP(SPP& data, unsigned int n, int seed = 1, int K=1, int B = 3, int delta = 100) {
	...
	data.A = n * (n - 1);
	std::stringstream dd;
	//dd << std::setprecision(2)<< (double)(delta / 100.0);
	//data.Delta = atof(dd.str().c_str());
	data.Delta = ((double)delta / 100.0);
	...
	data.solfilename = "SPP_n" + std::to_string(data.N) + "_s" + std::to_string(seed) + "_K" + std::to_string(data.K) + "_B" + std::to_string(data.B) +"_D"+std::to_string(ddid_version)+
		"_d"+std::to_string((int)(data.Delta * 100.0))
		+".log";
}
