...
struct KNP {
	...
	/** Cost of borrowing in the first-stage; useless for us */
	double ell;

	/** Cost of borrowing in the second-stage; useless for us. */
	double lambda;

	/*Number of risks factor*/
	unsigned short knp_nfactors = KNP_NFACTORS;

	/** Risk factor coefficients for costs: an array for each project. */
	std::vector<std::array<double, 1 + KNP_NFACTORS> > phi;

	/** Risk factor coefficients for profits: an array for each project */
...
static inline void gen_KNP(KNP& data, unsigned int n, int seed = 1, bool mixed_integer = false) {
	...
	data.ell = 0.12; // Useless for us.
	data.lambda = 1.2; // Useless for us.
	...
	for (n = 1; (int)n <= data.N; n++) {
		...
		// randomize in order to eliminate bias: old function used pre C++14
	...
	/*	std::random_device rng;
		...
		std::shuffle(data.phi[n].begin() + 1, data.phi[n].end(), urng);
		std::shuffle(data.ksi[n].begin() + 1, data.ksi[n].end(), urng);*/
